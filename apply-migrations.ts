...
async function applyMigrations() {
  try {
    ...
    // Instead, we need to do it via SQL Editor or use a different approach
    
    console.log('⚠️  SQL migration script generated.');
    console.log('📝 Please execute the following in Supabase SQL Editor:');
    console.log('═'.repeat(60));
    console.log(sql);
    console.log('═'.repeat(60));
    
    // Try creating first table as a test
    console.log('\n🧪 Testing Supabase connection with Organization table...');
    ...
    if (error?.code === 'PGRST116') {
      console.log('ℹ️  Table does not exist yet. Need to create schema manually.');
